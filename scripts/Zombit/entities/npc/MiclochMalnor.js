class MiclochMalnor extends MultiDialingNPC
{
	constructor(owningState)
	{
			let toSay =["<center><big>Micloch Malnor : Mais comment avais vous fait ?! Ce dialogue n'est pas censé être visible !.</big></center>",
    				"<center><big>Soldat : Eh bien je suis plutôt fort... J'ai soit trouvé un bug sois triché ;).</big></center>"];
    ...
    interact()
    {
    	...
    	if(this.dialogLevel == 2 && player.findKey)
    	{
    		...
    		this.texts = ["<center><big>Micloch Malnor : Vous en être sûr que c'est sécurisé ?</big></center>",
    	...
    	{
    		this.dialingText = "<center><big>Micloch Malnor : Mais enfin, c'est dangereux et rien ne me prouve que vous puissiez m'être utile et puis j'ai entendu plein de bruits qui font peur.</big></center>";

    		this.texts = ["<center><big>Soldat : Très bien restez la pendant que je fais le ménage dans l'étage. Mais après il faudrait me suivre dans la salle des serveurs pour pirater le laboratoire.</big></center>",
    					  "<center><big>Soldat : Tant que vous y êtes passer par la salle des techniciens, c'est là-bas que se trouve la clé qui ouvre la salle des serveurs.</big></center>",
    	...
    	{
    		this.dialingText = "<center><big>Micloch Malnor : Mais que faîtes vous ici ? Il y a une invasion de ces choses et je dois avouer que j'en ai une peur bleue...</big></center>";
